...
def show_colon(visible):
    display.image.set_pixel_value(2, 1, visible)
    display.image.set_pixel_value(2, 3, visible)


def get_clock_time():
    global base_time
    sys_time = system_time() / (1 + CLOCK_ADJUST)
...
def plot_LED_column(column, number):
    ...
    if number <= 5:
        for i in range(4, -1, -1):
            if i < 5 - number:
                display.image.set_pixel_value(column, i, 0)
            else:
                display.image.set_pixel_value(column, i, 1)
...
while True:
    ...
    if mode == 0:
        display_time = system_time() - last_display_time
