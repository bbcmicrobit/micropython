...
class ScoreState(object):
    ...
    def level_complete(self):
        if self._hits < 10:
            return
        image = Image("00000:00900:09990:00900:00000")
        display.show(image)
        sleep(300)
        display.clear()
        sleep(500)
        ...
        self._misses = 0
        self._hits = 0
        display.scroll("L{} S{}".format(self._level, self.score))
...
class BallState(object):
    ...
    def _test_collision(self):
        if self._row != 4:  # the ball is not low enough yet
            return
        if self._column == paddle_position or self._column == paddle_position + 1:  # player hit the ball
            self._hit()
        else:
            self._miss()
...
display.scroll("G/O S {}".format(my_score.score), loop=True)
